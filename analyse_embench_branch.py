...
def process_embench(embench_out_path, results_filename):
    ...
    embench_results.write('benchmark, branches, jrs, taken conditional branches')
    model_columns = []

    for bm in branch_models.keys():
        embench_results.write(f',{bm} mispredict')
    ...
    for embench_out_dir in embench_out_dirs:
        ...
        for bm in model_columns:
            embench_results.write(f",{branch_results[bm]['mispredict']}")
