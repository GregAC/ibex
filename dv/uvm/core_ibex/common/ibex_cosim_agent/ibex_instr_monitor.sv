...
class ibex_instr_monitor extends uvm_monitor;
  protected virtual core_ibex_rvfi_if vif;

  uvm_analysis_port#(ibex_instr_seq_item) item_collected_port;

  `uvm_component_utils(ibex_instr_monitor)
  `uvm_component_new
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual core_ibex_rvfi_if)::get(this, "", "vif", vif)) begin
      `uvm_fatal("NOVIF", {"virtual interface must be set for: ", get_full_name(), ".vif"});
    end
  endfunction: build_phase

  virtual task run_phase(uvm_phase phase);
    ibex_instr_seq_item trans_collected;
    ...
    forever begin
      ...
      trans_collected           = ibex_instr_seq_item::type_id::create("trans_collected");
