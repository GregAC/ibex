...
bool SpikeCosim::step(uint32_t write_reg, uint32_t write_reg_data,
                      uint32_t pc, bool sync_trap, bool suppress_reg_write) {
  assert(write_reg < 32);
...
bool SpikeCosim::check_retired_instr(uint32_t write_reg, uint32_t write_reg_data,
  ...
  for (auto reg_change : reg_changes) {
    ...
    if ((reg_change.first & 0xf) == 0) {
      ...
      if (!suppress_reg_write && !check_gpr_write(reg_change, write_reg,
                                                  write_reg_data)) {
        return false;
      }
...
bool SpikeCosim::check_suppress_reg_write(uint32_t write_reg, uint32_t pc,
  ...
  if (write_reg != 0) {
    ...
    err_str << "Instruction at " << std::hex << pc <<
      " indicated a suppressed register write but wrote to x" << std::dec <<
      write_reg;
    errors.emplace_back(err_str.str());
