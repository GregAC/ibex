...
def calc_branch_info(trace_lines, start_pc, stop_pc):
        ...
        if instr_line_match:
            ...
            if last_instr_branch_info:
                ...
                branches.append((last_instr_branch_info[0],
                    last_instr_branch_info[1], last_instr_branch_info[2], taken))

                if not last_instr_branch_info[2] and taken:
    ...
    return (branches, num_jrs, num_jmps, num_taken_conditional_branches)
...
class CounterTableBranchPredictModel:
    ...
    def predict_branch(self, pc, target, is_jump, taken):
        result = False
        ...
        if pc in self.branch_table:
            ...
            if is_jump:
                self.branch_table[pc] = (self.counter_max,
                        self.branch_table[pc][1])
...
def extract_bench_start_stop(trace_elf):
    try:
        ...
        raise(Exception('Failure calling nm ' + str(e)))
    except TimeoutExpired:
        raise(Exception('Timeout from nm'))
...
def run_branch_model(trace_filename, start_pc, stop_pc, branch_model):
    ...
    branch_info, num_jrs, num_jmps, num_taken_conditional_branches = calc_branch_info(trace_file_lines, start_pc, stop_pc)

    branches = len(branch_info)

    #t_p   = Taken branch, predicted correctly
    #t_mp  = Taken branch, mispredicted
    #nt_p  = Not taken branch, predicted correctly
    #nt_mp = Not taken branch, mispredicted
    t_p   = 0
    t_mp  = 0
    nt_p  = 0
    nt_mp = 0

    for (pc, target, is_jump, taken) in branch_info:
        prediction = branch_model.predict_branch(pc, target, is_jump, taken)

        assert((not is_jump) or taken)
