 ...
 */
PMP_WARN_UNUSED_RESULT
static bool pmp_cfg_permissions_set(pmp_region_config_t config,
                                    uint32_t *bitfield) {

  // Lock bit has a different meaning for the special shared regions
  pmp_region_lock_t lock = config.lock;
  ...
  switch (config.permissions) {
    ...
    case kPmpRegionPermissionsReadWriteExecute:
      ...
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_X, true);
      break;
    case kPmpRegionPermissionsReadWriteMachineReadOnlyUser:
      lock = kPmpRegionLockUnlocked;
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_W, true);
      break;
    case kPmpRegionPermissionsSharedReadWrite:
      lock = kPmpRegionLockUnlocked;
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_W, true);
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_X, true);
      break;
    case kPmpRegionPermissionsSharedExecuteOnly:
      lock = kPmpRegionLockLocked;
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_W, true);
      break;
    case kPmpRegionPermissionsReadExecuteMachineExecuteOnlyUser:
      lock = kPmpRegionLockLocked;
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_W, true);
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_X, true);
      break;
    case kPmpRegionPermissionsSharedReadOnly:
      lock = kPmpRegionLockLocked;
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_R, true);
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_W, true);
      *bitfield = bitfield_bit32_write(*bitfield, PMP_CFG_CSR_X, true);
  ...
  pmp_cfg_mode_lock_set(lock, bitfield);
...
pmp_region_configure_na4_result_t pmp_region_configure_na4(
  ...
  if (!pmp_cfg_permissions_set(config, &field_value)) {
